...
Enter the upper level common path of the 7 best model folders:/ mnt/data0/users/lisg/Data/brain2'
Input GPU: Example: 0 1 2 3
'''

def make_parser():
    parser = configargparse.ArgParser(description="Preprocessing eqtl data")
    parser.add_argument('--model', type=str, default='/mnt/data0/users/lisg/Data/brain')
    parser.add_argument('--gpu', type=int, nargs='+', help='gpu id')
    parser.add_argument('--caqtl', type=str, default='/mnt/data0/users/lisg/Data/caqtl',)
    return parser
def main():
    parser = make_parser()
    ...
    for Path in model_path:
        ...
        print('checkpoint:',bestmodel_path)
        ...
        test_data.close()
        print('predict caqtl')
        caqtl = ensure_trailing_slash(args.caqtl)
        output_path = os.path.join(Path,'caqtl')
        print(output_path)
        ...
        #
        positive = caqtl + 'positive.h5'
        negative_8 = caqtl + 'negative0.008.h5'
